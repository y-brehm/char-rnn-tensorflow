...
class Model:
    def __init__(self, args, training=True):
        ...
        if args.model == 'rnn':
            cell_fn = tf.compat.v1.nn.rnn_cell.RNNCell
        elif args.model == 'gru':
        ...
        if training and args.output_keep_prob:
            inputs = tf.compat.v1.nn.dropout(inputs, args.output_keep_prob)

        # unstack the input to fits in rnn model
        inputs = tf.split(inputs, args.seq_length, 1)
        inputs = [tf.squeeze(input_, [1]) for input_ in inputs]

        # loop function for rnn_decoder, takes the previous i-th cell's output and generate the (i+1)-th cell's input
        def loop(prev, _):
            prev = tf.matmul(prev, softmax_w) + softmax_b
            prev_symbol = tf.stop_gradient(tf.argmax(prev, 1))
            return tf.nn.embedding_lookup(embedding, prev_symbol)

        # rnn_decoder to generate the ouputs and final state. If not training the model, we use the loop function.
        outputs, last_state = tfa.seq2seq.BasicDecoder(inputs, self.initial_state, keras.layers.Bidirectional(keras.layers.RNN(cell)),
